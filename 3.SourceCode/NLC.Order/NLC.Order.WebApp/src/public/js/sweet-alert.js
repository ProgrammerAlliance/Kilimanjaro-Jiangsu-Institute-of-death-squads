...
(function(window, document) {
  ...
  let getModal = function() {
    ...
    getTopMargin = function(elem) {
      ...
      let padding = parseInt(getComputedStyle(elem).getPropertyValue('padding'), 10);
  ...
  window.sweetAlert = window.swal = function() {
    ...
    let $okButton = modal.querySelector('button.confirm'),
      $cancelButton = modal.querySelector('button.cancel'),
  ...
  function setParameters(params) {
    ...
    if (params.type) {
      ...
      let $icon = modal.querySelector('.icon.' + params.type);
      show($icon);
    ...
    // Custom image
    if (params.imageUrl) {
      let $customIcon = modal.querySelector('.icon.custom');
      ...
      if (params.imageSize) {
        let imgWidth = params.imageSize.split('x')[0];
        let imgHeight = params.imageSize.split('x')[1];
    ...
    // Done-function
    let hasDoneFunction = (params.doneFunction) ? true : false;
  ...
  function closeModal() {
    ...
    let $successIcon = modal.querySelector('.icon.success');
    removeClass($successIcon, 'animate');
    removeClass($successIcon.querySelector('.tip'), 'animateSuccessTip');
    removeClass($successIcon.querySelector('.long'), 'animateSuccessLong');

    let $errorIcon = modal.querySelector('.icon.error');
    removeClass($errorIcon, 'animateErrorIcon');
    removeClass($errorIcon.querySelector('.x-mark'), 'animateXMark');

    let $warningIcon = modal.querySelector('.icon.warning');
